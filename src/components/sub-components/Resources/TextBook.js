...
export class TextBook extends Component {
    render() {
        const {auth,profile,subject,medium} = this.props;
        var resultObject = search(subject, Data[medium]);
        return (
            <div className='routeArea book'>
            <span className='forumTitle TextBooks'>{subject} - {medium} - Grade 0{profile.grade}</span>
                <div className='chapterList'>
                    {
                     Object.keys(resultObject.Chapters).map(function(key) {
                            // console.log(key, resultObject.Chapters[key]);
                            return(
                            <a className='chapter' target="blank"  href={resultObject.Chapters[key]}
                            >{key}</a>
                            )
                        })
                    }
